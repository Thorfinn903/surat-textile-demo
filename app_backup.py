...
class Product(db.Model):
    ...
    wholesale_price = db.Column(db.Float, nullable=True) # New Field for B2B
    moq = db.Column(db.Integer, default=1)           # New Field for B2B
...
def ensure_product_columns():
    ...
    alter_statements = {
        ...
        'wholesale_price': 'ALTER TABLE product ADD COLUMN wholesale_price FLOAT', # B2B
        'moq': 'ALTER TABLE product ADD COLUMN moq INTEGER DEFAULT 4',           # B2B
    }
...
def admin():
    ...
    if request.method == 'POST':
        if 'add_product' in request.form:
            ...
            moq = request.form.get('moq', default=4, type=int)
...
def toggle_stock(id):
    ...
    product.stock = 'SOLD OUT' if product.stock == 'READY' else 'READY'
...
def bulk_upload():
    ...
    try:
        ...
        for row in csv_input:
            ...
            wholesale_price = row.get('wholesale_price', '').strip()
            moq = row.get('moq', '').strip()
